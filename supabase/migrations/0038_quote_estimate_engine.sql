-- Instant quote engine. Prices a system from catalog component picks + the rate
-- card (chargeables) + pricing_config margin + CER STC rules. Validated against
-- Pylon ($16,600) and SimPro ($16,672) on Jason Dawe. Called by HQ -> Instant quote.
-- AC-run band keys off inverter_kw (>10kW -> 16mm cable x 1.45); solar labour by
-- storey ($0.32/$0.34 per W, 6.6kW minimum); battery install tiered (16kWh/4-module
-- base +$180/module +$450/extra stack); STC solar = kW x zone x deeming, battery =
-- Cheaper Home Batteries tiers x 6.8.
...
declare
  ...
  v_admin numeric := 530; v_labour numeric; v_ac_cable numeric;
...
begin
  ...
  v_labour := v_solar_lab + v_dc + v_ac + v_bat_install + v_backup + v_admin;
  ...
  return jsonb_build_object(
    ...
    'battery_install', v_bat_install, 'backup', v_backup, 'admin', v_admin, 'labour_total', round(v_labour,2),
