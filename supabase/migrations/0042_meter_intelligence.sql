-- Meter photo AI learning -> quote.
-- The meter capture step now teaches the vision model to read three things and the
-- quote engine prices them:
--   1. old spinning-disc DIAL meter  -> a meter upgrade is required before solar.
--   2. smart-meter nameplate phase   -> 230V x1 = single, 230V x3 = three.
--   3. supply-authority SERVICE FUSE -> if absent, a Level 2 electrician is required
--      (external resource, ~$350-500).
-- validate-assessment-photo writes these into assessment_photos.ai_observations;
-- quote_prefill surfaces them so the instant quote is pre-flagged, not guessed.

alter table public.assessment_photos add column if not exists ai_observations jsonb;

-- Meter upgrade: the retailer swaps the meter, so no me-solar labour by default (0);
-- it is carried as a REQUIREMENT flag that must be actioned before install.
insert into public.chargeables (code, name, unit, category, baseline_rate, sort)
values ('meter_upgrade','Meter upgrade required (retailer smart-meter swap)','job','electrical',0,68)
on conflict (code) do nothing;

-- Level 2 service fuse install (no service fuse present): external Level 2 / ASP work,
-- ~$350-500. Baseline is the $425 midpoint; the range is recorded in meta.
insert into public.chargeables (code, name, unit, category, baseline_rate, sort, meta)
values ('service_fuse_l2','Level 2 service fuse install (no service fuse present)','job','electrical',425,69,
        '{"external":true,"low":350,"high":500,"note":"external Level 2/ASP electrician"}'::jsonb)
on conflict (code) do nothing;
...
declare v record; v_sd jsonb; v_aid uuid; v_obs jsonb;
begin
  if not (public.is_admin() or public.is_active_staff()) then raise exception 'not authorised'; end if;
  ...
  select id, site_data into v_aid, v_sd from assessments
    where lead_id = p_lead_id and status = 'completed' order by submitted_at desc limit 1;
  -- latest meter photo's AI-extracted facts (kind/phase/service fuse) for this visit
  select ai_observations into v_obs from assessment_photos
    where assessment_id = v_aid and step_key = 'meter' and ai_observations is not null
    order by created_at desc limit 1;
  return jsonb_build_object(
    'ss_ref', v.ss_ref, 'address', v.address, 'postcode', v.postcode,
    'phase', coalesce(v.phases,
                      case v_sd->>'phase' when 'three' then 3 when 'single' then 1 else null end,
                      nullif(v_obs->>'phase','')::int),
    ...
    'meter_kind', v_obs->>'meter_kind',
    'meter_upgrade', coalesce((v_obs->>'meter_upgrade_required')::boolean, v_sd->>'meter_type' = 'needs_upgrade', false),
    'service_fuse', v_obs->>'service_fuse',
    'service_fuse_l2', (v_obs->>'service_fuse' = 'absent'),
