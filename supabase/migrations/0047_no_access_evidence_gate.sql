-- A no-access claim must be a GENUINE condition, not an easy out. To submit no_access the
-- tech must prove all of:
--   1. Customer confirmation — the booking was confirmed (scheduled_at is set).
--   2. On-site GPS — they started the visit on site (start_gps), within 300 m of the site
--      (geofence against sites.lat/lng when we have it).
--   3. Geotagged evidence — at least one 'no_access_evidence' photo carrying GPS (photo of
--      the locked gate / no-one home, screenshot of the call log to the customer).
--   4. A written reason (already required).
-- Still a $0 outcome — this is about integrity, not payout.
...
begin
  ...
  -- no-access must be a genuine, evidenced condition
