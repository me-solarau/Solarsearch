-- Superannuation as a specified line item. Even a genuine sole-trader contractor is deemed
-- an employee FOR SUPER (Superannuation Guarantee (Administration) Act s12(3)) when the
-- contract is wholly or principally for their labour — which a regular assessment tech is.
-- So super is owed on the labour fee. We show it as its own line (fee + 12% super), not
-- baked into the rate, so the true cost is explicit and auditable per job.
-- (SG rate is 12% from 1 Jul 2025. Confirm the base/GST treatment with an accountant.)
alter table public.pricing_config add column if not exists super_rate_pct numeric(5,2) not null default 12;
alter table public.assessments add column if not exists super_cents int not null default 0;
...
declare
  ...
  v_super_rate numeric := coalesce((select super_rate_pct from pricing_config), 12);
  v_super int;
begin
  ...
  -- super is owed on the labour fee (s12(3) SGAA) — a specified line item, not baked in
  v_super := round(v_fee * v_super_rate / 100.0);

  update assessments set status = v_status, outcome = p_outcome, submitted_at = now(),
    fee_cents = v_fee, super_cents = v_super, site_data = coalesce(p_site_data, '{}'::jsonb)
  ...
  select l.site_id, v_a.lead_id, 'sales_rep', v_rep::text,
         ...
         jsonb_build_object('assessment_id', p_assessment_id, 'outcome', p_outcome,
                            'fee_cents', v_fee, 'super_cents', v_super, 'access_reason', v_reason,
                            'gps_dist_m', round(v_dist),
  ...
  return jsonb_build_object('ok', true, 'outcome', p_outcome, 'fee_cents', v_fee, 'super_cents', v_super,
