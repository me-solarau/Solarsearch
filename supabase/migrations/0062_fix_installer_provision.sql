-- Fix: decide_access() auto-provisioned installers against installers.user_id, but that
-- table links to the auth user via `auth_uid` (only sales_reps uses user_id). The installer
-- INSERT therefore threw and was swallowed by the exception handler ('auto-provision failed'),
-- granting the application but never creating the installer row. Use auth_uid for installers.
...
begin
  ...
  begin
    if v_app.app = 'sales_tech' then
      ...
      -- installers links via auth_uid (NOT user_id).
